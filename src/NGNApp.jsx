...
export default function NGNApp({ onHome }) {
  ...
  return(
    <div style={{background:"#020d18",minHeight:"100vh",fontFamily:"'Share Tech Mono','Courier New',monospace",color:"#c0d8f0",width:"100%",overflowX:"hidden"}}>
      ...
      <div style={{background:"linear-gradient(180deg,#091e30 0%,#020d18 100%)",borderBottom:"1px solid #00d4ff1a",padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
        <button onClick={onHome} style={{background:"#041520",border:"1px solid #00d4ff33",borderRadius:6,color:"#00d4ff",padding:"6px 12px",fontSize:12,flexShrink:0}}>← Home</button>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div className="hdr-sub" style={{fontSize:11,color:"#1a5a7a",marginTop:1}}>EVE-NG • OSPFv3 • BGP • Multicast • QoS • Cisco CME • FreePBX</div>
        </div>
      ...
      {/* INSTRUCTOR BAR */}
      <div className="inst-bar" style={{background:"#040f1c",borderBottom:"1px solid #0a2a40",padding:"7px 16px",display:"flex",gap:8,alignItems:"center",fontSize:11,flexWrap:"wrap"}}>
        <span>👨‍🏫</span>
        ...
        <div style={{marginLeft:"auto",background:"#0a1e2a",border:"1px solid #00d4ff1a",borderRadius:10,padding:"2px 10px",fontSize:10,color:"#4a8a9a",whiteSpace:"nowrap"}}>6 Modules · 25 Labs · 10 Đề Tài</div>
      </div>
      ...
      <div style={{display:"flex",borderBottom:"1px solid #0a2a40",background:"#030e1a",overflowX:"auto"}}>
        {[["topology","🗺","Topology"],["modules","📚","Modules & Labs"],["thesis","🎓","Đề Tài"],["guide","📖","Hướng Dẫn"]].map(([k,ic,l])=>(
          <button key={k} onClick={()=>setView(k)} className="nav-btn" style={{background:view===k?"#041a2a":"transparent",border:"none",borderBottom:view===k?"2px solid #00d4ff":"2px solid transparent",color:view===k?"#00d4ff":"#3a6a8a",padding:"10px 14px",fontSize:12,transition:"all 0.2s",flexShrink:0,whiteSpace:"nowrap"}}>
      ...
      <div style={{padding:"14px 14px",width:"100%"}}>
        ...
        {view==="topology"&&(
          <div className="fade">
            <div className="section-title" style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:11,color:"#3a6a8a",letterSpacing:3,marginBottom:10}}>▸ SƠ ĐỒ TOPOLOGY MẠNG</div>
            <div style={{display:"flex",gap:5,marginBottom:10,flexWrap:"wrap"}}>
              {TOPOS.map(t=>(
                <button key={t.id} onClick={()=>setTopo(t.id)} className="topo-btn" style={{background:topo===t.id?"#0a2a3a":"#050f18",border:`1px solid ${topo===t.id?"#00d4ff":"#1a3a5a"}`,color:topo===t.id?"#00d4ff":"#3a6a8a",padding:"6px 12px",borderRadius:5,fontSize:11,transition:"all 0.2s",whiteSpace:"nowrap"}}>{t.label}</button>
              ))}
            </div>
            <div style={{background:"#020d18",border:"1px solid #0a2a40",borderRadius:9,overflow:"hidden"}}>
              <div style={{background:"#030f1a",borderBottom:"1px solid #0a2a40",padding:"7px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{color:"#00d4ff",fontSize:12,fontFamily:"Rajdhani,sans-serif",fontWeight:700}}>{TOPOS.find(t=>t.id===topo)?.label}</span>
                <span style={{color:"#1a4a6a",fontSize:11}}>EVE-NG Lab</span>
              </div>
        ...
        {view==="modules"&&(
          <div className="fade">
            <div className="section-title" style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:11,color:"#3a6a8a",letterSpacing:3,marginBottom:12}}>▸ 6 MODULES · 25 LABS CHI TIẾT</div>
            {MODULES.map(mod=>(
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:2,flexWrap:"wrap"}}>
                        <span style={{color:mod.color,fontWeight:700,fontSize:12,letterSpacing:2}}>MODULE {mod.id}</span>
                        <span style={{color:"#1a4a6a",fontSize:11}}>|</span>
                        <span style={{color:"#3a6a8a",fontSize:11}}>{mod.labs}</span>
                      </div>
                    ...
                    <div className="mod-tags" style={{display:"flex",gap:4,flexWrap:"wrap",maxWidth:200,justifyContent:"flex-end",flexShrink:0}}>
                      {mod.tags.map(t=><span key={t} style={{background:"#020d18",border:`1px solid ${mod.color}40`,borderRadius:3,padding:"1px 6px",fontSize:10,color:mod.color}}>{t}</span>)}
                    </div>
                          ...
                          <div onClick={()=>{setExpLab(open?null:k);setLabTab("deploy");}}
                            ...
                            <div style={{flex:1,minWidth:0}}>
                              ...
                              <div style={{display:"flex",gap:8,marginTop:2}}>
                                <span style={{fontSize:10,color:"#3a5a7a"}}>⏱ {lab.dur}</span>
                                <span style={{fontSize:10,color:"#3a5a7a"}}>{"★".repeat(lab.diff)}{"☆".repeat(4-lab.diff)}</span>
                              </div>
                            </div>
                            <span style={{color:"#1a3a5a",fontSize:11,flexShrink:0}}>{open?"▲":"▼"}</span>
                          </div>
                            ...
                            <div className="fade" style={{padding:"12px 14px",background:"#020910",borderBottom:"1px solid #0a1828"}}>
                              <div style={{display:"flex",gap:4,marginBottom:10,flexWrap:"wrap"}}>
                                {[["deploy","🔧 Deploy"],["cmds","💻 Lệnh"],["topo","📐 Topo"],["result","✅ Kết Quả"],["hints","💡 Gợi Ý"]].map(([k2,l])=>(
                                  <button key={k2} onClick={e=>{e.stopPropagation();setLabTab(k2);}} style={{background:labTab===k2?`${mod.color}1a`:"#030c18",border:`1px solid ${labTab===k2?mod.color:"#0a1828"}`,color:labTab===k2?mod.color:"#3a6a8a",padding:"5px 10px",borderRadius:4,fontSize:11,whiteSpace:"nowrap"}}>{l}</button>
                                ))}
                              </div>

                              {labTab==="deploy"&&(
                                <div>
                                  <div style={{fontSize:11,color:mod.color,fontWeight:700,letterSpacing:1,marginBottom:8}}>HƯỚNG DẪN TRIỂN KHAI</div>
                                  {lab.deploy.map((st,i)=>(
                                    <div key={i} style={{display:"flex",gap:8,marginBottom:7,alignItems:"flex-start"}}>
                                      <div style={{width:20,height:20,borderRadius:"50%",background:`${mod.color}15`,border:`1px solid ${mod.color}50`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:mod.color,flexShrink:0,fontWeight:700}}>{i+1}</div>
                                      <span style={{fontSize:12,color:"#7a9aba",lineHeight:1.7}}>{st}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              {labTab==="cmds"&&(
                                <div>
                                  <div style={{fontSize:11,color:mod.color,fontWeight:700,letterSpacing:1,marginBottom:8}}>LỆNH CẤU HÌNH & VERIFY</div>
                                  <pre style={{background:"#010810",borderRadius:7,padding:"12px",fontSize:11,color:"#00e5aa",lineHeight:1.9,overflowX:"auto",borderLeft:`3px solid ${mod.color}`}}>{lab.cmds}</pre>
                                </div>
                              )}
                              {labTab==="topo"&&(
                                <div>
                                  <div style={{fontSize:11,color:mod.color,fontWeight:700,letterSpacing:1,marginBottom:8}}>TOPOLOGY LAB</div>
                                  <pre style={{background:"#010810",borderRadius:7,padding:"12px",fontSize:11,color:"#00d4ff",lineHeight:1.9,overflowX:"auto",borderLeft:`3px solid ${mod.color}`}}>{lab.topo}</pre>
                                </div>
                              )}
                              {labTab==="result"&&(
                                <div>
                                  <div style={{fontSize:11,color:mod.color,fontWeight:700,letterSpacing:1,marginBottom:8}}>KẾT QUẢ DỰ KIẾN</div>
                                  {lab.result.map((r,i)=>(
                              ...
                              {labTab==="hints"&&(
                                <div>
                                  <div style={{fontSize:11,color:"#f59e0b",fontWeight:700,letterSpacing:1,marginBottom:8}}>💡 GỢI Ý & TROUBLESHOOTING</div>
                                  {lab.hints.map((h,i)=>(
                                    <div key={i} style={{display:"flex",gap:8,marginBottom:6,background:"#0f0d02",border:"1px solid #2a1a00",borderRadius:4,padding:"7px 10px"}}>
                                      ...
                                      <span style={{fontSize:12,color:"#b09a4a"}}>{h}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
        ...
        {view==="thesis"&&(
          <div className="fade">
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,flexWrap:"wrap"}}>
              <div className="section-title" style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:11,color:"#3a6a8a",letterSpacing:3}}>{filtered.length} ĐỀ TÀI ĐỒ ÁN</div>
              <div style={{display:"flex",gap:5,marginLeft:"auto",flexWrap:"wrap"}}>
                {[["all","Tất Cả","#00d4ff"],["medium","Đồ Án Môn Học","#f59e0b"],["hard","NCKH/Luận Văn Tốt Nghiệp","#ff6b35"]].map(([k,l,c])=>(
                  <button key={k} onClick={()=>setFilter(k)} style={{background:filter===k?`${c}18`:"#050f18",border:`1px solid ${filter===k?c:"#1a3050"}`,color:filter===k?c:"#3a6a8a",padding:"5px 12px",borderRadius:4,fontSize:11,whiteSpace:"nowrap"}}>{l}</button>
                ))}
              </div>
            </div>
              ...
              <div key={t.id} style={{marginBottom:8}}>
                <div onClick={()=>{setExpThesis(expThesis===t.id?null:t.id);setThesisTab("overview");}}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:2,flexWrap:"wrap"}}>
                        <span style={{color:t.color,fontWeight:700,fontSize:12}}>ĐỀ TÀI {t.id}</span>
                        <span style={{background:`${t.color}15`,border:`1px solid ${t.color}40`,borderRadius:3,padding:"1px 7px",fontSize:10,color:t.color}}>{t.level==="medium"?"⭐⭐⭐ Đồ Án Môn Học":"⭐⭐⭐⭐⭐ NCKH/Luận Văn Tốt Nghiệp"}</span>
                      </div>
                      <div style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:14,color:"#d0e8ff"}}>{t.title}</div>
                      <div style={{display:"flex",gap:10,marginTop:2,flexWrap:"wrap"}}>
                        <span style={{fontSize:10,color:"#3a6a8a"}}>⏱ {t.dur}</span>
                        <span style={{fontSize:10,color:"#3a6a8a"}}>👥 {t.team}</span>
                      </div>
                    </div>
                  ...
                  <div className="fade" style={{background:"#030b16",border:`1px solid ${t.color}1a`,borderTop:"none",borderRadius:"0 0 8px 8px",padding:"14px"}}>
                    <div style={{display:"flex",gap:4,marginBottom:12,flexWrap:"wrap"}}>
                      {[["overview","📋 Tổng Quan"],["topo","📐 Topology"],["deploy","🔧 Kế Hoạch"],["result","✅ Kết Quả"]].map(([k,l])=>(
                        <button key={k} onClick={e=>{e.stopPropagation();setThesisTab(k);}} style={{background:thesisTab===k?`${t.color}1a`:"#030c18",border:`1px solid ${thesisTab===k?t.color:"#0a1828"}`,color:thesisTab===k?t.color:"#3a6a8a",padding:"5px 10px",borderRadius:4,fontSize:11,whiteSpace:"nowrap"}}>{l}</button>
                      ))}
                    </div>
                    ...
                    {thesisTab==="overview"&&(
                      <div>
                        ...
                        <div style={{fontSize:11,color:t.color,fontWeight:700,marginBottom:8}}>PHẠM VI</div>
                        {t.scope.map((sc,i)=>(
                          <div key={i} style={{display:"flex",gap:6,background:"#030c18",border:`1px solid ${t.color}1a`,borderRadius:4,padding:"7px 10px",marginBottom:5}}>
                            <span style={{color:t.color,fontSize:11,flexShrink:0}}>▸</span>
                            <span style={{fontSize:12,color:"#7a9aba",lineHeight:1.5}}>{sc}</span>
                          </div>
                        ))}
                        <div style={{marginTop:10,display:"flex",gap:5,flexWrap:"wrap"}}>
                          {t.tech.map(tc=><span key={tc} style={{background:"#020d18",border:`1px solid ${t.color}40`,borderRadius:3,padding:"2px 8px",fontSize:10,color:t.color}}>{tc}</span>)}
                        </div>
                      </div>
                    )}
                    {thesisTab==="topo"&&<pre style={{background:"#010810",borderRadius:7,padding:"12px",fontSize:11,color:"#00d4ff",lineHeight:1.9,overflowX:"auto",borderLeft:`3px solid ${t.color}`}}>{t.topo}</pre>}
                    {thesisTab==="deploy"&&(
                      <div>
                        {t.deploy.map((d,i)=>(
                          <div key={i} style={{display:"flex",gap:8,marginBottom:7}}>
                            <div style={{width:22,height:22,borderRadius:4,background:`${t.color}15`,border:`1px solid ${t.color}50`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:t.color,flexShrink:0,fontWeight:700}}>{i+1}</div>
                            <span style={{fontSize:12,color:"#7a9aba",lineHeight:1.7}}>{d}</span>
                          </div>
                        ))}
                      </div>
                    )}
        ...
        {view==="guide"&&(
          <div className="fade">
            <div className="section-title" style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:11,color:"#3a6a8a",letterSpacing:3,marginBottom:12}}>▸ HƯỚNG DẪN NHANH</div>
                ...
                <div key={i} style={{background:"#040f18",border:`1px solid ${card.c}22`,borderLeft:`3px solid ${card.c}`,borderRadius:7,padding:"12px"}}>
                  ...
                  {card.items.map((item,j)=>(
                    <div key={j} style={{display:"flex",gap:6,marginBottom:5}}>
                      <span style={{color:card.c,fontSize:11,flexShrink:0,marginTop:2}}>▸</span>
                      <code style={{fontSize:11,color:"#6a9aba",lineHeight:1.6,fontFamily:"Share Tech Mono,monospace",wordBreak:"break-all"}}>{item}</code>
                    </div>
                  ))}
                </div>
              ))}
                  ...
                  <thead>
                    <tr style={{background:"#030c18"}}>
                      {["Feature","Lệnh Verify","Kết Quả Mong Đợi"].map(h=>(
                        <th key={h} style={{padding:"7px 12px",textAlign:"left",fontSize:11,color:"#3a6a8a",fontWeight:700,borderBottom:"1px solid #0a2a40",whiteSpace:"nowrap"}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={i} style={{borderBottom:"1px solid #0a1828",background:i%2===0?"#030b16":"#020910"}}>
                        <td style={{padding:"7px 12px",fontSize:12,color:"#c0d8f0",whiteSpace:"nowrap"}}>{row[0]}</td>
                        <td style={{padding:"7px 12px"}}><code style={{fontSize:11,color:"#00e5aa",fontFamily:"Share Tech Mono,monospace"}}>{row[1]}</code></td>
                        <td style={{padding:"7px 12px",fontSize:11,color:"#5a8a7a"}}>{row[2]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* FOOTER */}
      <div style={{borderTop:"1px solid #0a2a40",padding:"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,color:"#1a4a6a",flexWrap:"wrap",gap:6}}>
